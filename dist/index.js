...
var DatabaseStorage = class {
  ...
  async createReminder(data) {
    const [reminder] = await this.db.insert(reminders).values(data).returning();
    return reminder;
  }
  async getUserReminders(crmUserId) {
    return await this.db.select().from(reminders).where(and(eq(reminders.crmUserId, crmUserId), eq(reminders.sent, "false"))).orderBy(reminders.reminderAt);
  }
  async getDueReminders() {
    return await this.db.select().from(reminders).where(and(eq(reminders.sent, "false"), sql2`${reminders.reminderAt} <= NOW()`));
  }
  async markReminderSent(id) {
    await this.db.update(reminders).set({ sent: "true" }).where(eq(reminders.id, id));
  }
  async deleteReminder(id) {
    const result = await this.db.delete(reminders).where(eq(reminders.id, id)).returning();
  ...
  app2.patch("/api/leads/:id", async (req, res) => {
    try {
      ...
      if (updateData.firstContacted && typeof updateData.firstContacted === "string") {
        const d = new Date(updateData.firstContacted);
        if (!isNaN(d.getTime())) updateData.firstContacted = d;
        else delete updateData.firstContacted;
      }
      if (updateData.lastContacted && typeof updateData.lastContacted === "string") {
        const d = new Date(updateData.lastContacted);
        if (!isNaN(d.getTime())) updateData.lastContacted = d;
        else delete updateData.lastContacted;
      }
  ...
  app2.patch("/api/clients/:id", async (req, res) => {
    try {
      ...
      if (updateData.lastContacted) {
        ...
        if (isNaN(updateData.lastContacted.getTime())) delete updateData.lastContacted;
      }
      if (updateData.updatedAt) delete updateData.updatedAt;
      if (updateData.leadAssignment === "") updateData.leadAssignment = null;
      if (updateData.coLeadAssignment === "") updateData.coLeadAssignment = null;
  ...
  app2.patch("/api/projects/:id", async (req, res) => {
    try {
      ...
      if (updateData.dueDate) {
        ...
        if (isNaN(updateData.dueDate.getTime())) delete updateData.dueDate;
      }
      if (updateData.updatedAt) delete updateData.updatedAt;
...
function registerBotRoutes(app2) {
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const activeLeads = leads2.filter((l) => l.isConverted === "false");
        res.json({
          success: true,
          leads: activeLeads
        });
  ...
  app2.get("/api/bot/user-clients/:platform/:platformUserId", authenticateBotRequest, async (req, res) => {
    try {
      const { platform, platformUserId } = req.params;
      const mapping = await storage.getBotUserMapping(platform, platformUserId);
      ...
      const clients2 = await storage.getClientsByOwner(user.id, user.role);
      res.json({ success: true, clients: clients2 });
    } catch (error) {
      res.status(500).json({ success: false, message: "Failed to get clients" });
    }
  });
  app2.patch("/api/bot/clients/:clientId/update", authenticateBotRequest, async (req, res) => {
    try {
  ...
  app2.delete("/api/bot/clients/:clientId", authenticateBotRequest, async (req, res) => {
    try {
      const { clientId } = req.params;
      const { platformUserId, platform } = req.body;
      const mapping = await storage.getBotUserMapping(platform, platformUserId);
  ...
  app2.post("/api/bot/leads/:leadId/notes", authenticateBotRequest, async (req, res) => {
    try {
      const { leadId } = req.params;
  ...
  app2.post("/api/bot/clients/:clientId/notes", authenticateBotRequest, async (req, res) => {
    try {
      const { clientId } = req.params;
      const { platformUserId, platform, note } = req.body;
      const mapping = await storage.getBotUserMapping(platform, platformUserId);
  ...
  app2.get("/api/bot/mystats/:platform/:platformUserId", authenticateBotRequest, async (req, res) => {
    try {
      const { platform, platformUserId } = req.params;
      const mapping = await storage.getBotUserMapping(platform, platformUserId);
      if (!mapping) return res.status(403).json({ success: false, message: "User not linked" });
      ...
      res.json({
        ...
        clients: {
          ...
          bySector: clients2.reduce((acc, c) => {
  ...
  app2.post("/api/bot/reminders", authenticateBotRequest, async (req, res) => {
    try {
  ...
  app2.get("/api/bot/reminders/due", authenticateBotRequest, async (req, res) => {
    try {
  ...
  app2.get("/api/bot/reminders/:platform/:platformUserId", authenticateBotRequest, async (req, res) => {
    try {
      const { platform, platformUserId } = req.params;
      const mapping = await storage.getBotUserMapping(platform, platformUserId);
      if (!mapping) return res.status(403).json({ success: false, message: "User not linked" });
  ...
  app2.delete("/api/bot/reminders/:reminderId", authenticateBotRequest, async (req, res) => {
    try {
...
\u2705 Converted to Clients: ${stats.converted}`
    ...
    this.bot.command("addnote", async (ctx) => {
      ...
      await ctx.reply("\u{1F4DD} Add a note to a:", { reply_markup: keyboard });
    });
    ...
    this.bot.command("mystats", async (ctx) => {
      ...
      try {
        const response = await fetch(
          `${this.config.apiUrl}/api/bot/mystats/telegram/${platformUserId}`,
          { headers: { "X-Bot-Key": this.config.botSecretKey } }
        );
        const data = await response.json();
        if (!data.success) {
          await ctx.reply("\u274C You're not linked yet. Use /link");
          return;
        }
        const { leads: leads2, clients: clients2, user } = data;
        const bySectorText = Object.entries(clients2.bySector).map(([s, n]) => `  ${s}: ${n}`).join("\n") || "  None";
        await ctx.reply(
          `\u{1F4CA} Stats for ${user.firstName} ${user.lastName}

\u{1F4CB} LEADS (${leads2.total} active)
\u251C\u2500 Initial Discussion: ${leads2.byStatus.initialDiscussion}
\u251C\u2500 NDA: ${leads2.byStatus.nda}
\u251C\u2500 Engagement: ${leads2.byStatus.engagement}
\u251C\u2500 Accepted: ${leads2.byAcceptance.accepted}
\u251C\u2500 Rejected: ${leads2.byAcceptance.rejected}
\u2514\u2500 Converted to Clients: ${leads2.converted}

\u{1F91D} CLIENTS (${clients2.total} total)
By Sector:
${bySectorText}`
        );
      } catch {
    ...
    this.bot.command("remind", async (ctx) => {
      ...
      const keyboard = new InlineKeyboard().text("Link to Lead", "remindtype_lead").text("Link to Client", "remindtype_client").row().text("No Link (free reminder)", "remindtype_none");
      await ctx.reply("\u{1F514} Set a reminder\n\nLink this reminder to a record?", { reply_markup: keyboard });
    });
  ...
  async handleCallbackQuery(ctx, session, data, userId) {
    switch (session.step) {
      ...
      case "note_select_record":
        ...
        sessions.set(userId, session);
        await ctx.reply("\u{1F4DD} Type your note:");
        break;
      // DELETE CLIENT FLOW
      ...
      case "remind_pick_type":
        ...
        if (session.data.linkedType === "lead") {
          ...
          sessions.set(userId, session);
          await ctx.reply("\u{1F4DD} What should I remind you about?");
        }
        break;
      ...
      case "remind_cancel":
        ...
        try {
          ...
          await ctx.answerCallbackQuery("\u274C Error cancelling reminder");
        }
        break;
    }
  }
  ...
  // ============================================
  async askLeadAssignment(ctx) {
    const keyboard = new InlineKeyboard().text("Pankaj Karna", "assign_Pankaj Karna").text("Nitin Gupta", "assign_Nitin Gupta").row().text("Aakash Jain", "assign_Aakash Jain").text("Ojasva Chugh", "assign_Ojasva Chugh").row().text("Ujjwal Jha", "assign_Ujjwal Jha").text("Devapi Singh", "assign_Devapi Singh");
    await ctx.reply("\u{1F465} Who should be the Lead?", { reply_markup: keyboard });
  }
  async askWantCoLead(ctx) {
    const keyboard = new InlineKeyboard().text("Yes", "colead_yes").text("No", "colead_no");
    await ctx.reply("Do you want to assign a Co-Lead?", { reply_markup: keyboard });
  }
  async askCoLeadAssignment(ctx) {
    const keyboard = new InlineKeyboard().text("Pankaj Karna", "coassign_Pankaj Karna").text("Nitin Gupta", "coassign_Nitin Gupta").row().text("Aakash Jain", "coassign_Aakash Jain").text("Ojasva Chugh", "coassign_Ojasva Chugh").row().text("Ujjwal Jha", "coassign_Ujjwal Jha").text("Devapi Singh", "coassign_Devapi Singh");
    await ctx.reply("\u{1F465} Who should be the Co-Lead?", { reply_markup: keyboard });
  }
  // ============================================
  // Link Account
  // ============================================
...
Please try again with /link`
  ...
  // Create Lead
  // ============================================
  async createLeadFinal(ctx, session, userId) {
    const platformUserId = ctx.from?.id.toString();
    const {
      companyName,
      sector,
    ...
    let summaryText = `\u{1F4CB} Creating lead...
...
Source: ${sourceType} - ${inboundSource || outboundSource}`;
    if (leadAssignment) {
      summaryText += `
\u{1F465} Lead: ${leadAssignment}`;
    }
    if (coLeadAssignment) {
      summaryText += `
\u{1F465} Co-Lead: ${coLeadAssignment}`;
    }
...
`;
        if (data.lead.leadAssignment) {
          successMessage += `\u{1F465} Lead: ${data.lead.leadAssignment}
`;
        }
        if (data.lead.coLeadAssignment) {
          successMessage += `\u{1F465} Co-Lead: ${data.lead.coLeadAssignment}
`;
        }
        successMessage += `
...
Use /newlead to add another lead.`;
  ...
  // Edit Lead Methods
  // ============================================
  async showLeadsToEdit(ctx, platformUserId) {
    try {
      const response = await fetch(
      ...
      if (!data.success || !data.leads || data.leads.length === 0) {
        await ctx.reply("\u{1F4CB} You don't have any leads to edit.");
        return;
      }
  ...
  async askWhatToEdit(ctx) {
    const keyboard = new InlineKeyboard().text("Status", "editfield_status").text("Acceptance Stage", "editfield_acceptance").row().text("Lead", "editfield_lead").text("Co-Lead", "editfield_colead");
    await ctx.reply("What would you like to edit?", { reply_markup: keyboard });
  }
  async askNewValue(ctx, field) {
    let keyboard = new InlineKeyboard();
    switch (field) {
      case "status":
        keyboard.text("Initial Discussion", "status_Initial Discussion").text("NDA", "status_NDA").row().text("Engagement", "status_Engagement");
        await ctx.reply("Select new status:", { reply_markup: keyboard });
        break;
      case "acceptance":
        keyboard.text("Undecided", "acceptance_Undecided").text("Accepted", "acceptance_Accepted").row().text("Rejected", "acceptance_Rejected");
        await ctx.reply("Select acceptance stage:", { reply_markup: keyboard });
        break;
      case "lead":
        keyboard.text("Pankaj Karna", "newlead_Pankaj Karna").text("Nitin Gupta", "newlead_Nitin Gupta").row().text("Aakash Jain", "newlead_Aakash Jain").text("Ojasva Chugh", "newlead_Ojasva Chugh").row().text("Ujjwal Jha", "newlead_Ujjwal Jha").text("Devapi Singh", "newlead_Devapi Singh");
        await ctx.reply("Select new lead:", { reply_markup: keyboard });
        break;
      case "colead":
        keyboard.text("Pankaj Karna", "newcolead_Pankaj Karna").text("Nitin Gupta", "newcolead_Nitin Gupta").row().text("Aakash Jain", "newcolead_Aakash Jain").text("Ojasva Chugh", "newcolead_Ojasva Chugh").row().text("Ujjwal Jha", "newcolead_Ujjwal Jha").text("Devapi Singh", "newcolead_Devapi Singh").row().text("Remove Co-Lead", "newcolead_");
        await ctx.reply("Select new co-lead:", { reply_markup: keyboard });
        break;
    }
  }
  async updateLeadField(ctx, session, userId) {
  ...
  // ============================================
  async showLeadsToConvert(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-leads/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      ...
      const data = await response.json();
      if (!data.success || !data.leads || data.leads.length === 0) {
        await ctx.reply("\u{1F4CB} You don't have any leads to convert.");
        return;
      }
      const keyboard = new InlineKeyboard();
      data.leads.slice(0, 10).forEach((lead, index) => {
  ...
  async confirmConversion(ctx, session) {
    const keyboard = new InlineKeyboard().text("Yes, Convert", "convert_yes").text("Cancel", "convert_no");
    await ctx.reply(
      "\u26A0\uFE0F Are you sure you want to convert this lead to a client?",
      { reply_markup: keyboard }
    );
  }
  async confirmDeletion(ctx, session) {
    const keyboard = new InlineKeyboard().text("Yes, DELETE", "delete_yes").text("Cancel", "delete_no");
    await ctx.reply(
      "\u26A0\uFE0F Are you sure you want to PERMANENTLY DELETE this lead? This action cannot be undone.",
      { reply_markup: keyboard }
    );
  }
  async convertLeadToClient(ctx, session, userId) {
...
\u{1F4CB} ${data.client.companyName}
View in Clients section.`
  ...
  async deleteLead(ctx, session, userId) {
    const { leadId } = session.data;
    const platformUserId = ctx.from?.id.toString();
    try {
      const response = await fetch(
  ...
  // Clients List & View
  // ============================================
  async showClientsList(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-clients/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      );
      const data = await response.json();
      if (!data.success || !data.clients || data.clients.length === 0) {
        await ctx.reply("\u{1F4CB} You don't have any clients yet.");
        return;
      }
      const lines = data.clients.slice(0, 15).map((c, i) => {
        const sector = c.customSector || c.sector || "\u2014";
        return `${i + 1}. *${c.companyName}* \u2014 ${sector}`;
      }).join("\n");
      await ctx.reply(`\u{1F91D} Your Clients (${data.clients.length}):

${lines}`, { parse_mode: "Markdown" });
    } catch {
      await ctx.reply("\u274C Failed to fetch clients.");
    }
  }
  async showLeadsToView(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-leads/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      ...
      const data = await response.json();
      if (!data.success || !data.leads || data.leads.length === 0) {
        await ctx.reply("\u{1F4CB} You don't have any leads.");
        return;
      }
      const keyboard = new InlineKeyboard();
  ...
  async showLeadDetail(ctx, leadId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-leads/telegram/${ctx.from?.id}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      );
      const data = await response.json();
      ...
      await ctx.reply(
        `\u{1F4CB} *${lead.companyName}*

\u{1F3ED} Sector: ${sector}
\u{1F4BC} Type: ${tx}
\u{1F464} POC: ${lead.clientPoc}
\u{1F4E7} Email: ${lead.emailId}
\u{1F4F1} Phone: ${lead.phoneNumber}
\u{1F4E5} Source: ${lead.sourceType} \u2014 ${source}
\u{1F4CA} Status: ${lead.status}
\u2705 Acceptance: ${lead.acceptanceStage}
\u{1F465} Lead: ${lead.leadAssignment || "\u2014"}
\u{1F465} Co-Lead: ${lead.coLeadAssignment || "\u2014"}`,
        { parse_mode: "Markdown" }
      );
    } catch {
      await ctx.reply("\u274C Failed to fetch lead details.");
    }
  }
  async showClientsToView(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-clients/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      ...
      const data = await response.json();
      if (!data.success || !data.clients || data.clients.length === 0) {
        await ctx.reply("\u{1F4CB} You don't have any clients.");
        return;
      }
      const keyboard = new InlineKeyboard();
      data.clients.slice(0, 10).forEach((c, i) => {
  ...
  async showClientDetail(ctx, clientId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-clients/telegram/${ctx.from?.id}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      );
      const data = await response.json();
      ...
      await ctx.reply(
        `\u{1F91D} *${client.companyName}*

\u{1F3ED} Sector: ${sector}
\u{1F4BC} Type: ${tx}
\u{1F464} POC: ${client.clientPoc || "\u2014"}
\u{1F4E7} Email: ${client.emailId || "\u2014"}
\u{1F4F1} Phone: ${client.phoneNumber || "\u2014"}
\u{1F465} Lead: ${client.leadAssignment || "\u2014"}
\u{1F465} Co-Lead: ${client.coLeadAssignment || "\u2014"}`,
        { parse_mode: "Markdown" }
      );
    } catch {
      await ctx.reply("\u274C Failed to fetch client details.");
    }
  }
  // ============================================
  // Edit Client
  // ============================================
  async showClientsToEdit(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-clients/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      ...
      const data = await response.json();
      if (!data.success || !data.clients || data.clients.length === 0) {
        await ctx.reply("\u{1F4CB} You don't have any clients to edit.");
        return;
      }
      const keyboard = new InlineKeyboard();
      data.clients.slice(0, 10).forEach((c, i) => {
  ...
  async askWhatToEditClient(ctx) {
    const keyboard = new InlineKeyboard().text("Lead", "editclientfield_lead").text("Co-Lead", "editclientfield_colead");
    await ctx.reply("What would you like to edit?", { reply_markup: keyboard });
  }
  async askNewClientValue(ctx, field) {
    const keyboard = new InlineKeyboard();
    if (field === "lead") {
      keyboard.text("Pankaj Karna", "clead_Pankaj Karna").text("Nitin Gupta", "clead_Nitin Gupta").row().text("Aakash Jain", "clead_Aakash Jain").text("Ojasva Chugh", "clead_Ojasva Chugh").row().text("Ujjwal Jha", "clead_Ujjwal Jha").text("Devapi Singh", "clead_Devapi Singh");
      await ctx.reply("Select new lead:", { reply_markup: keyboard });
    } else if (field === "colead") {
      keyboard.text("Pankaj Karna", "ccolead_Pankaj Karna").text("Nitin Gupta", "ccolead_Nitin Gupta").row().text("Aakash Jain", "ccolead_Aakash Jain").text("Ojasva Chugh", "ccolead_Ojasva Chugh").row().text("Ujjwal Jha", "ccolead_Ujjwal Jha").text("Devapi Singh", "ccolead_Devapi Singh").row().text("Remove Co-Lead", "ccolead_");
      await ctx.reply("Select new co-lead:", { reply_markup: keyboard });
    }
  }
  async updateClientField(ctx, session, userId) {
    ...
    try {
      ...
      await ctx.reply(`\u274C Failed to update ${fieldName}.`);
    }
  }
  // ============================================
  // Add Note
  // ============================================
  async showLeadsToAddNote(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-leads/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      ...
      const data = await response.json();
      if (!data.success || !data.leads || data.leads.length === 0) {
        await ctx.reply("\u{1F4CB} No leads found.");
        return;
      }
      const keyboard = new InlineKeyboard();
      data.leads.slice(0, 10).forEach((l, i) => {
  ...
  async showClientsToAddNote(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-clients/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      ...
      const data = await response.json();
      if (!data.success || !data.clients || data.clients.length === 0) {
        await ctx.reply("\u{1F4CB} No clients found.");
        return;
      }
      const keyboard = new InlineKeyboard();
      data.clients.slice(0, 10).forEach((c, i) => {
  ...
  async submitNote(ctx, session, userId) {
    const { noteType, recordId, noteText } = session.data;
    const platformUserId = ctx.from?.id.toString();
    const url = noteType === "lead" ? `${this.config.apiUrl}/api/bot/leads/${recordId}/notes` : `${this.config.apiUrl}/api/bot/clients/${recordId}/notes`;
    try {
  ...
  // Delete Client
  // ============================================
  async showClientsToDelete(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-clients/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      ...
      const data = await response.json();
      if (!data.success || !data.clients || data.clients.length === 0) {
        await ctx.reply("\u{1F4CB} No clients to delete.");
        return;
      }
      const keyboard = new InlineKeyboard();
      data.clients.slice(0, 10).forEach((c, i) => {
  ...
  async confirmClientDeletion(ctx, session) {
    const keyboard = new InlineKeyboard().text("Yes, Delete", "deleteclient_yes").text("Cancel", "deleteclient_no");
    await ctx.reply("\u26A0\uFE0F Are you sure you want to delete this client? This cannot be undone.", { reply_markup: keyboard });
  }
  async deleteClient(ctx, session, userId) {
    ...
    try {
      ...
      await ctx.reply("\u274C Failed to delete client.");
    }
  }
  // ============================================
  // Helper Functions
  ...
  parseReminderDate(input) {
    try {
      ...
      if (isNaN(dateIST.getTime()) || dateIST <= /* @__PURE__ */ new Date()) return null;
      const dateUTC = new Date(dateIST.getTime() - 5.5 * 60 * 60 * 1e3);
      return dateUTC;
    } catch {
      return null;
    }
  }
  async createReminder(ctx, session, userId) {
    const platformUserId = ctx.from?.id.toString();
...
\u{1F4DD} "${reminderMessage}"`, { parse_mode: "Markdown" });
      } else {
  ...
  async showMyReminders(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/reminders/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      );
      const data = await response.json();
      if (!data.success || !data.reminders || data.reminders.length === 0) {
        await ctx.reply("\u{1F514} You have no pending reminders.");
        return;
      }
      for (const r of data.reminders) {
        ...
        await ctx.reply(
          `\u{1F514} *${formatted}*
\u{1F4DD} ${r.message}${r.linkedType ? `
\u{1F517} Linked to ${r.linkedType}` : ""}`,
          { parse_mode: "Markdown", reply_markup: keyboard }
        );
      }
    } catch {
      await ctx.reply("\u274C Failed to fetch reminders.");
    }
  }
  async showLeadsToRemind(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-leads/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
  ...
  async showClientsToRemind(ctx, platformUserId) {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/user-clients/telegram/${platformUserId}`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
  ...
  async checkAndSendReminders() {
    try {
      const response = await fetch(
        `${this.config.apiUrl}/api/bot/reminders/due`,
        { headers: { "X-Bot-Key": this.config.botSecretKey } }
      );
      const data = await response.json();
...
\u{1F4DD} ${reminder.message}${reminder.linkedType ? `
\u{1F517} Linked to ${reminder.linkedType}` : ""}`,
            { parse_mode: "Markdown" }
          );
