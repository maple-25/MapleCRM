...
// Outreach Tracker Enums
export const outreachStatusEnum = pgEnum('outreach_status', ['draft', 'scheduled', 'sent', 'failed', 'bounced', 'opened', 'replied']);
export const outreachSourceEnum = pgEnum('outreach_source', ['fund_tracker', 'client_master_data', 'manual']);
...
// Outreach Emails table (individual emails in a campaign)
...
// Outreach Campaigns insert schema
...
// Bot User Mappings table - links Telegram/WhatsApp users to CRM users
export const botUserMappings = pgTable("bot_user_mappings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  platform: text("platform").notNull(), // 'telegram' or 'whatsapp'
  platformUserId: text("platform_user_id").notNull(), // Telegram user ID (e.g., "123456789")
  platformUsername: text("platform_username"), // @username (optional)
...
// Insert schema for bot user mappings
export const insertBotUserMappingSchema = createInsertSchema(botUserMappings).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  linkedAt: true,
...
// Export types
export type BotUserMapping = typeof botUserMappings.$inferSelect;
export type InsertBotUserMapping = z.infer<typeof insertBotUserMappingSchema>;
// Reminders table
export const reminders = pgTable("reminders", {
  ...
  crmUserId: varchar("crm_user_id").notNull().references(() => users.id),
  linkedType: text("linked_type"), // 'lead', 'client', or null
