import { 
  ...
  type OutreachCampaign, type InsertOutreachCampaign, type OutreachEmail, type InsertOutreachEmail,
  type BotUserMapping, type InsertBotUserMapping,
} from "@shared/schema";
import { drizzle } from "drizzle-orm/postgres-js";
import postgres from "postgres";
import { users, partners, leads, clients, projects, projectMembers, projectComments, clientComments, fundTracker, teamMembers, clientMasterData, userMasterDataPermissions, outreachCampaigns, outreachEmails, botUserMappings, reminders, InsertReminder } from "@shared/schema";
...
export interface IStorage {
  ...
  bulkCreateFunds(funds: InsertFundTracker[]): Promise<FundTracker[]>;
  ...
  updateOutreachCampaign(id: string, campaign: Partial<OutreachCampaign>): Promise<OutreachCampaign | undefined>;
  deleteOutreachCampaign(id: string): Promise<boolean>;
  
  // Outreach Emails
  getAllOutreachEmails(userId: string): Promise<OutreachEmail[]>;
  getOutreachEmailsByCampaign(campaignId: string): Promise<OutreachEmail[]>;
  ...
  bulkCreateOutreachEmails(emails: InsertOutreachEmail[]): Promise<OutreachEmail[]>;
  updateOutreachEmail(id: string, email: Partial<OutreachEmail>): Promise<OutreachEmail | undefined>;
  deleteOutreachEmail(id: string): Promise<boolean>;

  // Bot User Mappings
    getBotUserMapping(platform: string, platformUserId: string): Promise<BotUserMapping | undefined>;
    createBotUserMapping(data: InsertBotUserMapping): Promise<BotUserMapping>;
    updateBotUserMapping(platform: string, platformUserId: string, data: Partial<InsertBotUserMapping>): Promise<BotUserMapping | undefined>;
    deactivateBotUserMapping(platform: string, platformUserId: string): Promise<boolean>;
    getAllBotUserMappings(platform?: string): Promise<BotUserMapping[]>;
  // Reminders
  createReminder(data: InsertReminder): Promise<Reminder>;
  getUserReminders(crmUserId: string): Promise<Reminder[]>;
...
export class DatabaseStorage implements IStorage {
        ...
        if (matchingName) {
          ...
          const assignedLeads = await this.db.select().from(leads)
            .where(and(
              ...
              or(
                eq(leads.leadAssignment, matchingName as any),
                eq(leads.coLeadAssignment, matchingName as any)
              )
            ))
            .orderBy(desc(leads.updatedAt));
      ...
      if (userFirstName) {
        ...
        if (matchingName) {
          ...
          const assignedClients = await this.db.select().from(clients)
            .where(and(
              or(
                eq(clients.leadAssignment, matchingName as any),
                eq(clients.coLeadAssignment, matchingName as any)
              ),
              ne(clients.status, 'Transaction closed'),
              ne(clients.status, 'Client Dropped')
            ))
  ...
  async getLeadsByLeadAssignment(assigneeName: string, userRole: string): Promise<Lead[]> {
    if (userRole === 'admin') {
      return await this.db.select().from(leads).where(
        or(
          eq(leads.leadAssignment, assigneeName as any),
          eq(leads.coLeadAssignment, assigneeName as any)
        )
      );
    } else {
      return await this.db.select().from(leads).where(
  ...
  async getClientsByLeadAssignment(assigneeName: string, userRole: string): Promise<Client[]> {
    if (userRole === 'admin') {
      return await this.db.select().from(clients).where(
        or(
          eq(clients.leadAssignment, assigneeName as any),
          eq(clients.coLeadAssignment, assigneeName as any)
        )
      );
    } else {
      return await this.db.select().from(clients).where(
  ...
  async bulkCreateFunds(funds: InsertFundTracker[]): Promise<FundTracker[]> {
  ...
  async getAllOutreachCampaigns(userId: string): Promise<OutreachCampaign[]> {
    return await this.db.select()
      ...
      .where(eq(outreachCampaigns.createdBy, userId))
  ...
  async updateOutreachCampaign(id: string, campaign: Partial<OutreachCampaign>): Promise<OutreachCampaign | undefined> {
    const result = await this.db.update(outreachCampaigns)
  ...
  async getAllOutreachEmails(userId: string): Promise<OutreachEmail[]> {
    return await this.db.select()
      .from(outreachEmails)
  ...
  async getOutreachEmailsByCampaign(campaignId: string): Promise<OutreachEmail[]> {
    return await this.db.select()
      .from(outreachEmails)
      .where(eq(outreachEmails.campaignId, campaignId))
      .orderBy(desc(outreachEmails.createdAt));
  }
  ...
  async createOutreachEmail(email: InsertOutreachEmail): Promise<OutreachEmail> {
    const result = await this.db.insert(outreachEmails).values(email).returning();
    return result[0];
  }

  async bulkCreateOutreachEmails(emails: InsertOutreachEmail[]): Promise<OutreachEmail[]> {
    if (emails.length === 0) return [];
    const result = await this.db.insert(outreachEmails).values(emails).returning();
    return result;
  }

  async updateOutreachEmail(id: string, email: Partial<OutreachEmail>): Promise<OutreachEmail | undefined> {
    const result = await this.db.update(outreachEmails)
  ...
  // Bot User Mappings
  async getBotUserMapping(platform: string, platformUserId: string): Promise<BotUserMapping | undefined> {
    const result = await this.db.select().from(botUserMappings)
      .where(and(eq(botUserMappings.platform, platform), eq(botUserMappings.platformUserId, platformUserId)))
      .limit(1);
    return result[0];
  }

  async createBotUserMapping(data: InsertBotUserMapping): Promise<BotUserMapping> {
    const result = await this.db.insert(botUserMappings).values(data).returning();
    return result[0];
  }

  async updateBotUserMapping(platform: string, platformUserId: string, data: Partial<InsertBotUserMapping>): Promise<BotUserMapping | undefined> {
    const result = await this.db.update(botUserMappings)
      .set({ ...data, updatedAt: new Date() })
      .where(and(eq(botUserMappings.platform, platform), eq(botUserMappings.platformUserId, platformUserId)))
      .returning();
    return result[0];
  ...
  async deactivateBotUserMapping(platform: string, platformUserId: string): Promise<boolean> {
    try {
      await this.db.update(botUserMappings)
  ...
  async getAllBotUserMappings(platform?: string): Promise<BotUserMapping[]> {
    if (platform) {
  ...
  async getUserReminders(crmUserId: string): Promise<Reminder[]> {
    return await this.db.select().from(reminders)
      .where(and(eq(reminders.crmUserId, crmUserId), eq(reminders.sent, 'false')))
      .orderBy(reminders.reminderAt);
  }

  async getDueReminders(): Promise<Reminder[]> {
    return await this.db.select().from(reminders)
      .where(and(eq(reminders.sent, 'false'), sql`${reminders.reminderAt} <= NOW()`));
  }
