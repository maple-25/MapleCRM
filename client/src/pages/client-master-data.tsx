...
import { Plus, Edit, Trash2, CheckCircle, XCircle, AlertCircle, Database, Search, Download } from "lucide-react";
            ...
            <Card data-testid="card-master-data">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle>Master Data Entries ({masterData.filter((data) => {
                    if (!searchTerm) return true;
                    const searchLower = searchTerm.toLowerCase();
                    return (
                      data.name?.toLowerCase().includes(searchLower) ||
                      data.designation?.toLowerCase().includes(searchLower) ||
                      data.company?.toLowerCase().includes(searchLower) ||
                      data.industry?.toLowerCase().includes(searchLower) ||
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {masterData.filter((data) => {
                        ...
                        <TableRow key={data.id} data-testid={`row-master-data-${data.id}`}>
